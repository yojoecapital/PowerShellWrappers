...
namespace PowerShellWrappers.Engine
{
    ...
    {
        public static void Move(IEnumerable<(string source, string destination)> pathPairs)
        {
            ...
            if (result != 0)
            {
                throw new InvalidOperationException($"Error moving: {GetErrorMessage(result)}");
            }
        }

        public static void MergeMove(IEnumerable<(string source, string destination)> pathPairs)
        {
            // Convert the list of path pairs into null-terminated strings
            var fromPathsBuilder = new StringBuilder();
            var toPathsBuilder = new StringBuilder();
            ...
            var fileOp = new SHFILEOPSTRUCT
            {
                ...
                pFrom = fromPathsBuilder.ToString(),
                pTo = toPathsBuilder.ToString(),
                fFlags = FILEOP_FLAGS.FOF_ALLOWUNDO | FILEOP_FLAGS.FOF_NOCONFIRMATION | FILEOP_FLAGS.FOF_NOCONFIRMMKDIR | FILEOP_FLAGS.FOF_MULTIDESTFILES
            };
